...
// Sprint 4: Added Interview and InterviewFeedback to migration for interview stats queries
func setupDashboardTestDB(t *testing.T) *DashboardHandler {
	...
	if err := db.AutoMigrate(
		...
		&models.Application{},
		&models.Candidate{},
		&models.Interview{},         // Sprint 4: required for interview stats queries
		&models.InterviewFeedback{}, // Sprint 4: required for completed interviews JOIN
	); err != nil {
...
func TestGetDashboardStats_EmptyDatabase(t *testing.T) {
	...
	if stats.TotalInterviews != 0 {
		t.Errorf("Expected totalInterviews 0, got %d", stats.TotalInterviews)
	}
...
// ─── Sprint 4: Interview Statistics Tests ─────────────────────────────────────

// TestGetDashboardStats_TotalInterviewsCount verifies correct total interview count
func TestGetDashboardStats_TotalInterviewsCount(t *testing.T) {
	handler := setupDashboardTestDB(t)

	// Create 3 interviews
	for i := 0; i < 3; i++ {
		handler.DB.Create(&models.Interview{
			ApplicationID: uint(i + 1),
			InterviewerID: 1,
	...
	if stats.TotalInterviews != 3 {
		t.Errorf("Expected totalInterviews 3, got %d", stats.TotalInterviews)
	}
}

// TestGetDashboardStats_PendingAndCompletedInterviews verifies pending vs completed split
func TestGetDashboardStats_PendingAndCompletedInterviews(t *testing.T) {
	handler := setupDashboardTestDB(t)
	...
	for i := 0; i < 3; i++ {
		...
		handler.DB.Create(&interview)
		interviews = append(interviews, interview)
	}

	// Submit feedback for 1 interview only (making it completed)
	handler.DB.Create(&models.InterviewFeedback{
	...
	if stats.CompletedInterviews != 1 {
		t.Errorf("Expected completedInterviews 1, got %d", stats.CompletedInterviews)
	}
	if stats.PendingInterviews != 2 {
		t.Errorf("Expected pendingInterviews 2, got %d", stats.PendingInterviews)
	}
}

// TestGetDashboardStats_InterviewFieldsInResponseShape verifies Sprint 4 camelCase fields exist
func TestGetDashboardStats_InterviewFieldsInResponseShape(t *testing.T) {
	handler := setupDashboardTestDB(t)
	...
	// Verify exact camelCase field names in JSON response (confirmed with team)
	var raw map[string]interface{}
	...
	for _, field := range requiredFields {
		if _, exists := raw[field]; !exists {
			t.Errorf("Expected Sprint 4 field '%s' in response, not found", field)
		}
	}
}
